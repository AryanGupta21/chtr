...
class StackedRNN(nn.Module):
    def __init__(self, input_size=128, hidden_size=64, output_size=2, num_layers=4):
...
def compute_metrics(y_true, y_pred):
    ...
    conf_mat_crackle = confusion_matrix(y_true[:, 0], y_pred_bin[:, 0])
    conf_mat_wheeze = confusion_matrix(y_true[:, 1], y_pred_bin[:, 1])
    return precision, recall, f1, conf_mat_crackle, conf_mat_wheeze
...
def eval_model(model, loader, loss_fn, device):
    ...
    precision, recall, f1, conf_mat_crackle, conf_mat_wheeze = compute_metrics(y_true, y_pred)
    return avg_loss, precision, recall, f1, conf_mat_crackle, conf_mat_wheeze
...
model = StackedRNN(num_layers=3).to(device)
...
for epoch in range(num_epochs):
    ...
    val_loss, precision, recall, f1, conf_mat_crackle, conf_mat_wheeze = eval_model(
    ...
    confmat_final_crackle = conf_mat_crackle
    confmat_final_wheeze = conf_mat_wheeze
